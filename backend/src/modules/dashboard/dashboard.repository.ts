...
export class DashboardRepository {
  ...
  public async listRecentActivities(companyId: string, page: number, limit: number) {
    ...
    const [rows, totalRow] = await Promise.all([
      db
        ...
        .limit(limit)
        .offset(offset),
      db.select({ value: count() }).from(auditLogs).where(whereClause).then((rows) => rows[0]?.value ?? 0)
    ]);
  ...
  public async getAlertData(companyId: string) {
    const [inventoryRows, customerDueRows, supplierDueRows, payrollRows, gstRows, notificationRows] = await Promise.all([
      db
        .select({
  ...
  public async getPendingTaskData(companyId: string) {
    const [draftSales, draftPurchases, pendingPayments, unpaidSalary, gstPending] = await Promise.all([
      db
        .select({
          count: count(),
    ...
    const customerPendingPayments = pendingPayments[0][0] ?? { count: 0, amount: "0.00" };
    const supplierPendingPayments = pendingPayments[1][0] ?? { count: 0, amount: "0.00" };

    return {
      draftSales: draftSales[0] ?? { count: 0, amount: "0.00" },
      draftPurchases: draftPurchases[0] ?? { count: 0, amount: "0.00" },
      pendingPayments: {
        count: customerPendingPayments.count + supplierPendingPayments.count,
        amount: (
          Number(customerPendingPayments.amount ?? 0) + Number(supplierPendingPayments.amount ?? 0)
        ).toFixed(2)
      },
  ...
  public async getSnapshotData(companyId: string, month: PeriodBounds) {
    const [inventory, gst, payroll, accounting] = await Promise.all([
      db
        .select({
          totalProducts: sql<number>`count(distinct ${products.id})`,
    ...
    const [itcRow, expiryRow] = await Promise.all([
      db
        .select({
