...
const buildPdfContent = (dataset: ReportExportDataset) => {
  ...
  const renderPageBase = (pageNumber: number, totalPages: number) => {
    ...
    commands.push(drawText(pageWidth - marginX - 64, footerY, `Page ${pageNumber} / {{TOTAL_PAGES}}`, { size: 8, color: PDF_MUTED }));
  ...
  const startNewPage = () => {
    ...
    const base = renderPageBase(currentPageNum, 0);
    currentCommands = base.commands;
    currentY = base.cursorY;
    currentPageNum++;
  };
  ...
  let tableY = currentY - headerHeight;
  currentCommands.push(drawRect(marginX, tableY - tableHeaderRowHeight, contentWidth, tableHeaderRowHeight, PDF_ACCENT, PDF_ACCENT));
  let columnX = marginX;
  ...
  bodyRows.forEach((row, rowIndex) => {
    if (currentY - row.height < marginBottom + footerHeight + 10) {
      startNewPage();
  ...
  if (dataset.secondaryTable) {
    const secColumns = dataset.secondaryTable.columns;
    const secRows = dataset.secondaryTable.rows.map((row, idx) => ({
      srNo: idx + 1,
      values: secColumns.map((col) => toDisplayValue(row[col.key], col))
    }));
    const secTableColumns = [
      { key: "__sr", label: "No.", type: "number" as const, values: secRows.map((r) => String(r.srNo)) },
      ...secColumns.map((col, colIdx) => ({
        key: col.key,
        label: col.label,
        type: col.type ?? "string",
        values: secRows.map((r) => r.values[colIdx] ?? "")
      }))
    ];
    const secWeights = secTableColumns.map((col) => {
    ...
    const initialSecSpace = titleHeight + secTableHeaderRowHeight + 15;
    
    if (currentY - initialSecSpace < marginBottom + footerHeight + 10) {
      startNewPage();
    } else {
      currentY -= 15;
    }

    currentCommands.push(drawText(marginX, currentY - 14, dataset.secondaryTable.title, { size: 11, font: "F2", color: PDF_TEXT }));
    currentY -= titleHeight;

    currentCommands.push(drawRect(marginX, currentY - secTableHeaderRowHeight, contentWidth, secTableHeaderRowHeight, PDF_ACCENT, PDF_ACCENT));
    let columnX = marginX;
    ...
    secBodyRows.forEach((row, rowIndex) => {
      if (currentY - row.height < marginBottom + footerHeight + 10) {
        ...
        currentCommands.push(drawText(marginX, currentY - 14, `${dataset.secondaryTable!.title} (Contd.)`, { size: 11, font: "F2", color: PDF_TEXT }));
        currentY -= titleHeight;
  ...
  if (dataset.notes) {
    const wrappedNotes = wrapText(dataset.notes, Math.floor(contentWidth / (tableFontSize * 0.5)));
    const notesBoxHeight = wrappedNotes.length * lineHeight + 18;
    
    if (currentY - (notesBoxHeight + 20) < marginBottom + footerHeight + 10) {
      startNewPage();
    } else {
      currentY -= 15;
    }

    currentCommands.push(drawRect(marginX, currentY - notesBoxHeight, contentWidth, notesBoxHeight, PDF_HEADER_FILL, PDF_BORDER));
    currentCommands.push(drawText(marginX + 8, currentY - 14, "Notes:", { size: 8.5, font: "F2", color: PDF_MUTED }));
    
    wrappedNotes.forEach((line, lineIdx) => {
      currentCommands.push(drawText(marginX + 8, currentY - 26 - lineIdx * lineHeight, line, { size: 8.5, color: PDF_TEXT }));
    });
    
    currentY -= notesBoxHeight;
  }

  // Draw Terms if present
  if (dataset.terms) {
    const wrappedTerms = wrapText(dataset.terms, Math.floor(contentWidth / (tableFontSize * 0.5)));
    const termsBoxHeight = wrappedTerms.length * lineHeight + 18;
    
    if (currentY - (termsBoxHeight + 20) < marginBottom + footerHeight + 10) {
      startNewPage();
    } else {
      currentY -= 15;
    }

    currentCommands.push(drawRect(marginX, currentY - termsBoxHeight, contentWidth, termsBoxHeight, PDF_HEADER_FILL, PDF_BORDER));
    currentCommands.push(drawText(marginX + 8, currentY - 14, "Terms & Conditions:", { size: 8.5, font: "F2", color: PDF_MUTED }));
    
    wrappedTerms.forEach((line, lineIdx) => {
      currentCommands.push(drawText(marginX + 8, currentY - 26 - lineIdx * lineHeight, line, { size: 8.5, color: PDF_TEXT }));
    });
  ...
  const finalPageStreams = pageStreams.map((stream) => stream.replaceAll("{{TOTAL_PAGES}}", String(pageStreams.length)));

  const objects: string[] = [];
  objects.push("<< /Type /Catalog /Pages 2 0 R >>");
  const pageRefs = finalPageStreams.map((_, index) => `${5 + index * 2} 0 R`).join(" ");
  objects.push(`<< /Type /Pages /Count ${finalPageStreams.length} /Kids [${pageRefs}] >>`);
  objects.push("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>");
  objects.push("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>");

  finalPageStreams.forEach((stream) => {
