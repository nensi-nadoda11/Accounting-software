...
import type { SalesPaymentStatus, SalesPriceTaxType } from "./sales.types";
...
type InvoiceCalculationInput = {
  ...
  deliveryCharges?: string | number | null | undefined;
  packingCharges?: string | number | null | undefined;
  otherCharges?: string | number | null | undefined;
  roundOffEnabled?: boolean | undefined;
};

type ReturnCalculationInput = {
  items: Array<{
...
const calculateLineValues = (input: LineCalculationInput) => {
  ...
  const lineTotalMoney = input.priceTaxType === "inclusive" ? discountedTotal : discountedTotal + gstAmountMoney + cessAmountMoney;
...
export const calculateInvoiceTotals = (input: InvoiceCalculationInput) => {
  ...
  const lines = rawLines.map((line, index) => {
    ...
    const lineTotal =
      input.items[index]!.priceTaxType === "inclusive"
        ? addDecimals(netTaxable, addDecimals(bigIntToMoney(gstAmount), bigIntToMoney(cessAmount), 2), 2)
        : addDecimals(netTaxable, addDecimals(bigIntToMoney(gstAmount), bigIntToMoney(cessAmount), 2), 2);
  ...
  const beforeRoundOff = addDecimals(
    addDecimals(addDecimals(lineGrandTotal, deliveryCharges, 2), packingCharges, 2),
    otherCharges,
    2
  );
...
export const calculateReturnTotals = (input: ReturnCalculationInput) => {
  ...
  const roundOffAmount = input.roundOffEnabled === false ? "0.00" : calculateRoundOff(grandTotalBeforeRoundOff);
  const grandTotal = addDecimals(grandTotalBeforeRoundOff, roundOffAmount, 2);
